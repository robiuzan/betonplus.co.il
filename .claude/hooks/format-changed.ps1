...
try {
    ...
    foreach ($line in $status) {
        ...
        # Skip deletions -- the path no longer exists, and handing it to Prettier
        # prints "No files matching the pattern were found" to stderr.
    ...
    $targets = $files |
        Where-Object { $_ -match $exts -and $_ -notmatch '(^|/)(node_modules|\.next|out|out\.prev|dist|build|public|vendor|content)/' } |
