...
| 1.2 | `app/sitemap.ts` emits **no `lastModified`** at all. Not wrong, but a real per-route date is a free freshness signal. (Build-time stamps would be worse than nothing.)                                                                                                                                  | 🟡  |
...
| 4.4 | `CollectionPage` / `AboutPage` / `ContactPage` absent on the index and static routes (breadcrumb only).                                                                                                             | ⚪  |
...
| #   | Item                                                                                                                                                                                                | Sev |
| --- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --- |
| 5.1 | **There is no location silo.** 16 areas exist as strings in `lib/site.ts`, rendered as chips linking nowhere. Zero pages target `<service> ב<city>`.                                                | 🟠  |
| 5.2 | Coverage contradiction: the visible list includes ירושלים and מודיעין; `schema.areaServed` says גוש דן והמרכז; the FAQ claims פריסה ארצית (and the hero card shows "ארצי"). Pick one truth (owner). | 🟠  |
...
| 7.3 | **No human is named anywhere** — no owner, no foreman, no technician.                                                                                                                                                                                                                                                                                                                       | 🟠  |
...
| 10.2 | Fonts load via a render-blocking `<link>` to Google Fonts with `preconnect` but **no `rel="preload"`** — FOUT on first paint on every page. Since the hero is a CSS gradient the `<h1>` is the likely LCP element, putting fonts on the LCP path. | 🟡  |
...
| #    | Item                                                                                                                                                                                                                                                 | Sev |
| ---- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --- |
| 13.1 | The GTM script renders as the **first child of `<body>`** (`app/layout.tsx`), not in `<head>`. Delays container load and can miss early events.                                                                                                      | 🟡  |
...
5. **§5.1** — build the location silo, but only once §3 is done and §5.2 is resolved.
