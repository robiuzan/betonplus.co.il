...
/**
 * Faithfully replays the page's original scripts (jQuery → theme/plugin libraries → inline
 * config → init) IN DOCUMENT ORDER, after the ported DOM is mounted.
 *
 * The body markup is injected via dangerouslySetInnerHTML (React treats it as opaque), so
 * this code enhances it exactly as it did on the live WordPress site. Scripts run
...
declare global {
  interface Window {
    __siteScriptsStarted?: boolean;
  }
}

export default function ThemeScripts({ scripts }: { scripts: ScriptItem[] }) {
  useEffect(() => {
    if (window.__siteScriptsStarted) return;
    window.__siteScriptsStarted = true;
