...
const isExternal = (href: string) => /^(tel:|mailto:|https?:)/.test(href) || href.startsWith("//");
  ...
  return (
    <Link href="/" className="flex items-center gap-2.5" aria-label="בטון פלוס — דף הבית">
      ...
      <span className="leading-tight">
        <span
          className={`block font-heading text-xl font-extrabold ${light ? "text-white" : "text-brand"}`}
        >
          בטון פלוס
        </span>
        <span
          className={`block text-[11px] font-semibold ${light ? "text-white/70" : "text-steel"}`}
        >
