 ...
 *
 * ⚠️ Architecture note (see CLAUDE.md): when the source theme renders page DESIGN
 * server-side from post-meta the REST API does NOT expose (`content.rendered` empty for
 * inner pages), the rendered pages are built from `content/site.json` — a snapshot of the
 * LIVE HTML (see lib/content.ts). Verify whether the `zapo` theme behaves this way.
 *
...
export const WP_URL = (process.env.NEXT_PUBLIC_WP_URL ?? "https://betonplus.co.il").replace(/\/+$/, "");
